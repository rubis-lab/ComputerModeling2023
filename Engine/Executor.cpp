...
bool Executor::run_simulation(JobVectorOfSimulator& job_vector_of_simulator, JobVectorsForEachECU& job_vectors_for_each_ECU, double start_time)
{
    ...
    for(auto job : job_vector_of_simulator)
    {
        if(job->get_actual_start_time() < 0 || job->get_actual_finish_time() > job->get_actual_deadline())
        {
            ...
            global_object::logger->real_cyber_schedule_logger_201914388(utils::current_time, job->get_job_id(), "DEADLINE MISS");
        }
    }
    ...
    while(std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now() - hyper_period_start).count() < utils::hyper_period)
    {
            ...
            if((job->get_det_prdecessors().size() == 0) && ( job->get_is_simulated() == false) && (job->get_is_released() == false))
            {
                if(job->get_is_read())
                {
                    ...
                    else
                    {
                        ...
                        is_idle = false;   
                        // std::thread t(&Logger::real_cyber_schedule_logger_201914388, global_object::logger, job->get_simulated_release_time(), job->get_job_id(), "RELEASED");
                        // t.detach();
                        global_object::logger->real_cyber_schedule_logger_201914388(job->get_simulated_release_time(), job->get_job_id(), "RELEASED");
                    }
                }
                else
                {
        ...
        else
        {
            ...
            // std::thread t(&Logger::real_cyber_schedule_logger_201914388, global_object::logger, run_job->get_simulated_start_time(), run_job->get_job_id(), "STARTED");
            // t.detach();
            global_object::logger->real_cyber_schedule_logger_201914388(run_job->get_simulated_start_time(), run_job->get_job_id(), "STARTED");
